...
class Wordle:
    ...
    .hard_mode          Set equal to true to consider any guesses (even those 
                        you know are false). Slower (~15-20 second per guess).
    ...
    def __init__(self):
        # self.words will be the list of remaining possible answers 
        # self.all_words is all viable answers at the start of the game
    ...
    def guess_outcomes(self):
        ...
        if self.hard_mode : 
            guess_list=self.words 
        else:
            guess_list=self.all_words
        ...
        if not self.hard_mode: 
            output['viable_ans'] = output['guess'].isin(self.words)
            output = order_front(output, ['guess','viable_ans'])
            minimum = output['avg_remaining'].min()
            output = output.query('guess in @self.words | avg_remaining <= @minimum + 0.5')
        ...
        for let,colors in let_colors_dict.items():
            if len(colors) > 1:
                if 'b' in colors:
                    ...
                    self.words = [w for w in self.words if w.count(let) == non_b]                    
                else:
                    self.words = [w for w in self.words if w.count(let) >= len(colors)]
